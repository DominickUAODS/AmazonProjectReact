  ...
  export default function ProductDetails({ product, categoryId }: Props) {
    ...
    useEffect(() => {
        if (!product && categoryId) {
            console.log("[ProductDetails] Загружаем propertyKeys для категории:", categoryId);
            fetch(`${import.meta.env.VITE_API_SERVER}/category/${categoryId}`)
                .then((res) => res.json())
                .then((data) => {
                    console.log("[ProductDetails] Данные категории:", data);

                    setPropertyKeys(data.propertyKeys || []);
                    setDetails([]); // очищаем, когда меняется категория
                    setNextIndex(0);
                });
        }
    }, [product, categoryId]);
            ...
            {!product ? (
                <div className={styles.addProductDetail}>
                    <button
                        ...
                        disabled={nextIndex >= propertyKeys.length} // если ключи кончились, дизейблим
                    >
                        <svg
                            width="28"
                            height="29"
                    ...
                    {details.map((detail, index) => (
                        <OnePropertyKeyAdmin
                            ...
                            onAttributeChange={(val) =>
                                handleAttributeChange(index, val)
                            }
                        />
                    ))}
                </div>
