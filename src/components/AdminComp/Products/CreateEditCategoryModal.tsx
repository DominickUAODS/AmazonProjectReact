...
const CreateEditCategoryModal: React.FC<CreateEditCategoryModalProps> = ({ show, onClose, onCreate, category, parentId }) => {
	...
	const [, setFullCategory] = useState<Category | null>(null);
	...
	const handleDeletePropertyKey = (index: number) => {
		setPropertyKeys(prev => prev.filter((_, i) => i !== index));
	};
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		// if (category) {
		// 	// редактирование — сохраняем текущего родителя
		// 	resolvedParentId = category.parent_id ?? null;
		// } else if (parentId) {
		...
		onCreate({
			...
			image,
			propertyKeys: cleanedPropertyKeys,
		});
		onClose();
	};
	...
	const { title: mainTitle } = getLabels();
										...
										<div className={styles.dropdown}>
											{IconOptions.map((option) => (
												<div
													...
													className={`${styles.dropdownItem} ${selected.value === option.value ? styles.active : ""
														}`}
												>
													{option.icon}
												</div>
									...
									<div className={styles.pkList}>
										{propertyKeys.map((pk, idx) => (
											...
											<OnePropertyKey
												...
												name={pk.name}
												isReadOnly={!!category}
												onChange={newName => handlePropertyKeyChange(idx, newName)}
												onDelete={() => handleDeletePropertyKey(idx)}
											/>
										))}
